 ...
 */
public class EfficientMessageStatsReport extends Report implements MessageListener {
	private double latency_sum;
	private long hopCount_sum;
	private double msgBufferTime_sum;
	private double rtt_sum; // round trip times
	 ...
	 * Constructor.
	 */
	public EfficientMessageStatsReport() {
		init();
	}
	...
	public void messageDeleted(Message m, DTNHost where, boolean dropped) {
		...
		this.msgBufferTime_sum += getSimTime() - m.getReceiveTime();
	}
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		this.nrofRelayed++;
		if (finalTarget) {
			this.latency_sum += getSimTime() - m.getCreationTime();
			this.nrofDelivered++;
			this.hopCount_sum += m.getHopCount();

			if (m.isResponse()) {
				this.rtt_sum += getSimTime() -	m.getRequest().getCreationTime();
	...
	public void done() {
		...
		String statsText = "created: " + this.nrofCreated +
			...
			"\nlatency_avg: " + format(latency_sum / nrofDelivered) +
			"\nhopcount_avg: " + format((double) hopCount_sum / nrofDelivered) +
			"\nbuffertime_avg: " + format(msgBufferTime_sum / (nrofDropped + nrofRemoved)) +
			"\nrtt_avg: " + format(rtt_sum / nrofResponseDelivered)
			;
